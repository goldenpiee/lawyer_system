...
class Appointment(models.Model):
    ...
    client = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='client_appointments', verbose_name="Клиент")
    lawyer = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='lawyer_appointments', verbose_name="Юрист")
    date = models.DateTimeField("Дата и время", help_text="Дата и время записи")
    ...
    class Meta:
        ...
        ordering = ['-date']
        constraints = [
            models.UniqueConstraint(fields=['lawyer', 'date'], name='unique_appointment')
        ]
    ...
    @property
    def can_cancel(self):
        return self.status in ['Pending', 'Approved'] and timezone.now() < (self.date - timedelta(hours=24))

class CalendarSlot(models.Model):
    lawyer = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='calendar_slots', verbose_name="Юрист")
    start_time = models.DateTimeField("Начало", help_text="Начало временного слота")
    end_time = models.DateTimeField("Конец", help_text="Конец временного слота")
    ...
    class Meta:
        ...
        ordering = ['start_time']
        constraints = [
            models.UniqueConstraint(fields=['lawyer', 'start_time'], name='unique_slot')
        ]
